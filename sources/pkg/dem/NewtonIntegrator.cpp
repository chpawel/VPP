...
void NewtonIntegrator::updateEnergy(const shared_ptr<Body>& b, const State* state, const Vector3r& fluctVel, const Vector3r& f, const Vector3r& m){
	...
	Real tmpEnergy = 0;
	State* st = dynamic_cast<State*>(b->state.get());
	...
	if(damping!=0. && state->isDamped){
		...
		//energia PCH do state

		Real eneVel = fluctVel.cwiseAbs().dot((f+state->mass*gravity).cwiseAbs())*damping*scene->dt;
		Real eneAngVel = state->angVel.cwiseAbs().dot(m.cwiseAbs())*damping*scene->dt;
		
		st->dEnergyYade +=  eneVel+eneAngVel;


		st->dEnergyVel += eneVel;
		st->dEnergyAngVel += eneAngVel;
