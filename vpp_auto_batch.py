...
frictAngle = atan(0.)# po ulozeniu kul zmiana tarcia.
frictAngleFoil = 0 #tarcie folii
ratio_upak =  0.54 #ratio upakowania /ile kul ma byc wygenerowanych 




srednica_probki = 0.051 *1 #/2   # [m]
wysokosc_probki = 0.105 *1# [m]
...
#########################################
nazwaPliku = "wyniki/pilaPol_v2_upak_54_kn_1_engFoil_tar_0_dt_0p25_"+str(pressureValue)+"_"+str(l_kul)+"_"+str(wys)+"_"
nazwaPlikuIter = "wyniki/pilaIterPol_v2_upak_54_kn_1_engFoil_tar_0_dt_0p25_"+str(pressureValue)+"_"+str(l_kul)+"_"+str(wys)+"_"
############### Parametry zapisu c++ ########
...
amplituda = 0.01 #6*promien_k#0.01 #0.05 * wysokosc_probki #1*promien_f # [m]
omega = 2.84 * 5  # [1/s]
time = 0. # [s]
idGora = 0 # id sciany do dociskania


velPila = 0.000833333 * 2 # [m/s] predkosc obnizania przy wymuszeniu pila maszyna 50 mm na min
posPila = amplituda/2 #start ruchu

direct = -1 #kierunek ruchu -1 do dolu + do gory


vObnizenia = 0.005 *50 # [m/s]
...
timeToStartPressure = 0.05 #czas ktory jest odczekiwany do startu po wlaczeniu cisnienia [s]
dtimeToStart = 0 # [s]
tmpDtimeToStart = 0 #tmp dla czasu - czas z kroku poprzedniego [s]
stage = 0 #0 - usypywanie, 2 - obnizenie sciany , 4 - ruch calosci

relGora = -promien_f # obnizenie gornej sciany przy dociskaniu
relFolia = 0.005 *2 # ile mm obnizenia folii i sciany (tak jak przy wkladaniu do maszyny)
...
O.materials.append(VppCohFrictMat(young=young_f, poisson=poisson_f, density=density_f, frictionAngle=radians(0),normalCohesion=nch, shearCohesion=sch, momentRotationLaw=False, knRatio=1,foilType=1, label='folia'))
	...
	if stage == 1:
		if O.bodies[idFoliiStart].state.refPos[2] - O.bodies[idFoliiStart].state.pos[2] < relFolia:
			for k in range(idFoliiStart,idFoliiKoniec+1):
				O.forces.addMove(k,(0,0,-vObnizenia*O.dt)) #obnizanie poprzez zadanie przemieszzcenia
				#O.bodies[k].state.vel[2]=-vObnizenia #zmiana bo addMove ma blad// zmiana predkosci
...
def PreChangePos():
	global stage
	if stage == 2:
		#O.bodies[idGora].state.vel[2]=-vObnizenia #wymuszenie predkoscia
		O.forces.addMove(idGora,(0,0,-vObnizenia*O.dt)) #wymuszenie polozeniem
...
def ChangePosPila():
		...
		velZ = direct * velPila
                ds = Vector3(0,0,dz)
                for k in range(idFoliiStart,idFoliiKoniec+1):
                        O.forces.addMove(k,(ds)) #wymuszenie poprzez zadanie polozenia
			#O.bodies[k].state.vel[2]=velZ #wymuszenie poprzez zadanie predkosci
                O.forces.addMove(idGora,(ds)) #wymuszenie poprzez zadanie polozenia
		#O.bodies[idGora].state.vel[2]=velZ #wymuszenie poprzez zadanie predkosci
...
print("DT do zapisu")
step = O.dt
PiterSave = 100 #ile zapisow w pliku
PtimeSr = 100*step
PtimeStart = 2.6 *0  #start zapisu
PdeltaTime = (2*(20/2)-(PtimeSr * PiterSave) ) / PiterSave
...
TimeStartAllIter =10000  
DeltaTimeAllIter= 50*(PtimeSr + PdeltaTime)

TimeStartAllBody =10000*2.6
DeltaTimeAllBody= 20*(PtimeSr + PdeltaTime)



pauseOn = True # jezeli true to wlaczy sie pausa symaulacji - zmiana na false pozwoli na uruchomienie recznie

savedFiles = 3 # ile ma byc wykonanych zapisow EndTime musi byc wiekszy niz czas do zapisow
EndTime = 10000#PtimeStart + IleZapisow *(((PiterSave) * (PdeltaTime+PtimeSr))+PdeltaTime)
...
O.engines=[
	...
	VppSave(label='saveBody',info = info, fileName=nazwaPliku, iterSave= PiterSave, timeSr = PtimeSr, timeStart = PtimeStart, deltaTime= PdeltaTime,timeStartAll = TimeStartAllBody,deltaTimeAll = DeltaTimeAllBody,idMinAll=1330, idMaxAll=1340),
   ForceResetter(),
   VppNeighbours(),
   VppPressureForce(),
   ...
   InteractionLoop(
		...
		VppIp2_CohFrictMat_CohFrictMat_CohFrictPhys(setCohesionNow=True,setCohesionOnNewContacts=True),
