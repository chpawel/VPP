...
#include"VppNewtonIntegrator.hpp"
#include<core/Scene.hpp>
#include<core/Clump.hpp>
#include<lib/base/Math.hpp>


YADE_PLUGIN((VppNewtonIntegrator));
CREATE_LOGGER(VppNewtonIntegrator);

// 1st order numerical damping
void VppNewtonIntegrator::cundallDamp1st(Vector3r& force, const Vector3r& vel){
	for(int i=0; i<3; i++) force[i]*=1-damping*Mathr::Sign(force[i]*vel[i]);
}
// 2nd order numerical damping
void VppNewtonIntegrator::cundallDamp2nd(const Real& dt, const Vector3r& vel, Vector3r& accel){
	for(int i=0; i<3; i++) accel[i]*= 1 - damping*Mathr::Sign ( accel[i]*(vel[i] + 0.5*dt*accel[i]) );
}

Vector3r VppNewtonIntegrator::computeAccel(const Vector3r& force, const Real& mass, int blockedDOFs){
...
Vector3r VppNewtonIntegrator::computeAngAccel(const Vector3r& torque, const Vector3r& inertia, int blockedDOFs){
...
void VppNewtonIntegrator::updateEnergy(const shared_ptr<Body>& b, const State* state, const Vector3r& fluctVel, const Vector3r& f, const Vector3r& m){
...
void VppNewtonIntegrator::saveMaximaVelocity(const Body::id_t& id, State* state){
	#ifdef YADE_OPENMP
...
void VppNewtonIntegrator::saveMaximaDisplacement(const shared_ptr<Body>& b){
...
void VppNewtonIntegrator::action()
{		cout << "licze energie start" << endl;	
	scene->forces.sync();
	...
	YADE_PARALLEL_FOREACH_BODY_BEGIN(const shared_ptr<Body>& b, scene->bodies){
			...
			if(b->isClump()) Clump::moveMembers(b,scene,this);
				...
				for(size_t i=0; i<callbacksSize; i++){
					...
					//cout<< callbacks[i]->scene->forces.getForce(b->id)<<endl;
				}
			#endif
...
void VppNewtonIntegrator::leapfrogTranslate(State* state, const Body::id_t& id, const Real& dt){
...
void VppNewtonIntegrator::leapfrogSphericalRotate(State* state, const Body::id_t& id, const Real& dt )
{
...
void VppNewtonIntegrator::leapfrogAsphericalRotate(State* state, const Body::id_t& id, const Real& dt, const Vector3r& M){
...
bool VppNewtonIntegrator::get_densityScaling() {
...
void VppNewtonIntegrator::set_densityScaling(bool dsc) {
	FOREACH(const shared_ptr<Engine> e, Omega::instance().getScene()->engines) {
		GlobalStiffnessTimeStepper* ts=dynamic_cast<GlobalStiffnessTimeStepper*>(e.get());
		if (ts) {
...
Quaternionr VppNewtonIntegrator::DotQ(const Vector3r& angVel, const Quaternionr& Q){
	Quaternionr dotQ;
