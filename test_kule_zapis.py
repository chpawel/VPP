...
frictAngle = atan(0.6)# po ulozeniu kul zmiana tarcia.
...
#########################################
nazwaPliku = "wyniki/test_Zapis_"
nazwaPlikuIter = "wyniki/stIter_Zapis_"
############### Parametry zapisu c++ ########
...
relFolia = 0.005 # ile mm obnizenia folii i sciany (tak jak przy wkladaniu do maszyny)
...
O.materials.append(VppCohFrictMat(young=young_f, poisson=poisson_f, density=density_f, frictionAngle=radians(0.6),normalCohesion=nch, shearCohesion=sch, momentRotationLaw=False, knRatio=1,foilType=1, label='folia'))

O.materials.append(FrictMat(young=1e9, poisson=.25, frictionAngle=0, density=1000, label='frictionlessWalls'))

O.materials.append(CohFrictMat(young=young_k, poisson=poisson_k, density=density_k, frictionAngle=radians(0.6),normalCohesion=0, shearCohesion=0, momentRotationLaw=False, label='kula'))
...
####test kul
O.bodies.append([ utils.sphere(center=(0,0,0),radius=promien_k,fixed=True,material=kula[0],color=(0,0,1),mask=mask_k),])
O.bodies.append([ utils.sphere(center=(0,0,2*2*promien_k),radius=promien_k,fixed=False,material=kula[0],color=(0,0,1),mask=mask_k),])
...
print("DT do zapisu")
step = O.dt
PiterSave =50 #ile zapisow w pliku
PtimeSr = 100*step
PtimeStart = 0.0 #start zapisu
PdeltaTime = 1000*step
...
TimeStartAllBody =0.
DeltaTimeAllBody= 10*(PtimeSr + PdeltaTime)
...
O.engines=[
	...
	VppSave(label='saveBody',info = info, fileName=nazwaPliku, iterSave= PiterSave, timeSr = PtimeSr, timeStart = PtimeStart, deltaTime= PdeltaTime,timeStartAll = TimeStartAllBody,deltaTimeAll = DeltaTimeAllBody,idMinAll=0, idMaxAll=1),
   ForceResetter(),
   VppNeighbours(),
   VppPressureForce(),
